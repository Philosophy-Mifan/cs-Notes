...
##### 4.4.1 深度优先搜索 Depth-first search
...
##### 4.4.2 广度优先搜索 Breadth-first search
...
<img src="Picture/4-16-1 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

以上图（左）为例，按照两个顶点之间边的权值大小得出数组（右表所示）。

按照权值大小，先添加`0-5`这一条边

<img src="Picture/4-16-1-1 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

随后，加入`0-1`这条边

<img src="Picture/4-16-1-2 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

同理，过程图如下：

<img src="Picture/4-16-1-3 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

<img src="Picture/4-16-1-4 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

<img src="Picture/4-16-1-5 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

<img src="Picture/4-16-1-6 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />

剩下的边都会使生成树成环，直接删除即可。

<img src="Picture/4-16-1-7 克鲁斯卡尔算法.png" alt="1" style="zoom:50%;" />



每添加一条边X——Y，判断X和Y的终点是否一致，且顶点的终点就是连通的最大顶点，按照权值最小的原则可以算出下列图及数组，类似并查集

<img src="Picture/4-16-2 克鲁斯卡尔算法.png" style="zoom:50%;" />
...
<img src="Picture/4-16-2-1 Prim.png" alt="1" style="zoom:50%;" />
...
<img src="Picture/4-17-1 拓扑.png" style="zoom:40%;" />
...
<img src="Picture/4-17-2 拓扑.png" style="zoom:40%;" />

2、将栈顶元素弹出，且更新入度

<img src="Picture/4-17-3 拓扑.png" style="zoom:40%;" />
<img src="Picture/4-17-4 拓扑.png" style="zoom:40%;" />

3、同1，然后循环操作，只要有入度为0的点则自动入栈

<img src="Picture/4-17-5 拓扑.png" style="zoom:40%;" />

<img src="Picture/4-17-6 拓扑.png" style="zoom:48%;" />

<img src="Picture/4-17-7 拓扑.png" style="zoom:48%;" />

<img src="Picture/4-17-8 拓扑.png" style="zoom:57%;" />
...
先把V0（源点）周围的路径权值记录一遍，要注意，在初始化权值的时候一定是inf（无穷大），若初始化为0，则与求最短路径冲突，是一种不合理的行为。将V0相邻的最小权值的点纳入路径中，并在final数组里标为1
...
<img src="Picture/4-22 关键路径.png" style="zoom:50%;" />

- 事件的最早发生时间 ETV：从源点开始计算，如上图$V_2$的最早发生时间为6，$V_6$的最早发生时间为7。
- 事件的最晚发生时间 LTV：从汇点倒推，如果超出了这个时间，整个工期都会延误，如$V_2$的最晚发生时间为$18-12=6$，可以发现其与最早发生时间一致，这也是关键路径的性质；$V_6$的最晚发生时间为$18-7=11$。
- 活动的最早开工时间 ETE：就是活动起点事件的最早发生时间，如$V_2→V_5$的最早开工时间就是$V_2$的最早发生时间。
- 活动的最晚开工时间 LTE：就是活动终点事件的最晚发生时间减去当前活动所需的时间，如$V_6→V_8$的最晚开工时间就是$V_8$的最晚发生时间减去$V_6→V_8$所需的时间。

最终我们要求的是活动的最早开工时间和最晚开工时间，两者相等时，则说明该路径为关键路径。

我们用上图来解释一下关键路径求解的步骤：
要注意在计算时入度为0才能计算，且在计算时需要比较，留下最大值的ETV记录
1、用拓扑排序求出ETV，在计算时要保证每一个事件的入度（邻接表）均为0后再遍历下一个（如$V_5$需要把$V_2$和$V_3$遍历后才遍历，以保证计算的准确性），表初始化为下，这里不会显式的将邻接表输出，只看计算过程即可。ETV的计算公式为：$ETV[x]=ETV[x→front]+V(x→front,x)$，如$ETV[V_3]=ETV[1]+V(V_1,V_3)=0+4=4$
上图拓扑排序序列为：$V_1→V_4→V_6→V_3→V_2→V_5→V_8→V_7→V_9$

<img src="Picture/4-22-1 ETV.png" style="zoom:70%;" />

将$V_2$、$V_3$和$V_4$放入栈中，计算得

<img src="Picture/4-22-2 ETV.png" style="zoom:70%;" />

将$V_5$和$V_6$放入栈中，计算得

<img src="Picture/4-22-3 ETV.png" style="zoom:70%;" />

将$V_7$和$V_8$放入栈中，计算得

<img src="Picture/4-22-4 ETV.png" style="zoom:70%;" />

将$V_9$放入栈中，计算得

<img src="Picture/4-22-5 ETV.png" style="zoom:70%;" />
...
<img src="Picture/5-2 InterpolationSearch.png" style="zoom:67%;" />
...
从磁盘提取数据的方式（速度慢，I/O操作）：以块读取，B-树通过在一个结点中存放多个数据已达到降低树高度，从而降低I/O操作次数，提高整体搜素速度。

B-树是一种平衡的多路查找树，它在文件系统中很有用；针对外存，处理硬盘、磁盘中的数据。

1. B-树最多可以存多少个？最少可以存多少个？	A:有上界和下界（除根结点以外）
2. 结点的最小度数t：t取决于磁盘块的大小，每个结点至少$t-1$个，最多$2t-1$个（工程上自取即可）
3. 如果包含$x$个关键字，就有$x+1$个孩子
4. 每个结点的关键字要升序排列，孩子就类似于二叉排序树。

<img src="Picture/5-12-1 B-tree.png" style="zoom:35%;" />

以上图为例，$t=3$，下界为2，上界为5。

在B-树的插入操作中，当一个结点的个数达到上界时，取出这个结点的最中间元素向父亲结点插入，其余结点平均分布成两个结点，如下图所示，这个过程叫做主动分裂。

<img src="Picture/5-12-2 B-tree.png" style="zoom:45%;" />

高级树的删除操作普遍偏难，在B-树中的删除操作可以分为以下几种：

1. 待删除元素在叶子结点中，在保证符合B-树定义的条件下直接删除（特殊情况见3）

2. 待删除元素在内部结点（非终端结点、非叶子结点）

   1. 如果位于$x$中的关键字$k$之前的孩子有至少$t$（下界+1）个关键字，就在孩子结点中找到$k$的直接前驱，并替换掉。如下图，删除G元素，此时G之前的孩子有3个结点（≥3），故找到G的直接前驱F，将其替换G的位置即可。

      <img src="Picture/5-12-3 B-tree.png" style="zoom:45%;" />

   2. 如果位于$x$中的关键字$k$之前的孩子少于$t$个关键字，且$x$中的关键字$k$之后的孩子有至少$t$个关键字，就在孩子结点中找到$k$的直接后驱，并替换掉。
      如下图，删除元素，此时G之前的孩子有1个结点（＜3），故找到的直接后驱F，将其替换G的位置即可。

      <img src="Picture/5-12-4 B-tree.png" style="zoom:45%;" />

   3. 如果位于$x$中的关键字$k$之前的孩子少于$t$个关键字，且$x$中的关键字$k$之后的孩子也少于$t$个关键字，此时在删除操作之前先进行合并操作，后属于第1种情况，直接删除即可。
      如下图，合并ABCDE为一个结点，再将C删除

      <img src="Picture/5-12-5 B-tree.png" style="zoom:45%;" />

3. [特殊]待删除关键字$k$在叶子结点中，删除后不符合B-树定义，且父结点及其兄弟都只有$t-1$个关键字，此时为了防止<u>级联效应</u>会自主的进行合并，如下图：

   <img src="Picture/5-12-6 B-tree.png" style="zoom:45%;" />

   合并后，待删除关键字$k$在叶子结点中，此时查看其兄弟结点是否有富余，若有，则进行类似于“旋转”的操作，也就是将兄弟结点的临近关键字移到上方，将父结点原有的“隔断”关键字替换待删除关键字即可，如下图：

   <img src="Picture/5-12-7 B-tree.png" style="zoom:45%;" />



#### 3.1.5 B+树

B-树的过程是提出一整块磁盘块的数据（I/O操作）后，再进行多路查找，在B-树中，需要存储指向磁盘的指针，其一个结点的数据是有限的。

1、让大部分结点能够使用满，而不被其他的变量占用（将真实数据的指针放在叶子中）内部结点充当索引

2、更好的区间查询

B+树是多级动态索引结构，关系型数据库的“索引”（SQL），结点内存储自带地址无需使用额外空间（指针）

对于第1点，如下图所示

<img src="Picture/5-13-1 B+tree.png" style="zoom:45%;" />

对于第2点，其所有的数据元素都在叶子结点上，且从左到右全部有序，对所有的叶子结点间接入指针，使其变成一整个单链表。

与同高度的B-树相比，B+树存放的数据更多，亦得到，当具有同样的数据量时，B+树的高度是小于等于B-树的高度的

- 插入操作
  1. 先插入到叶子，不能破坏大小顺序
  2. 结点有明确的范围，如果超出要先分裂再插入，关键字范围：$\lceil m/2 \rceil≤n≤m$，n是关键字个数，m是阶数，根节点则为$1≤n≤m$。

如：对上图插入数据95，其变化如下所示（只展示右下部分）

<img src="Picture/5-13-2 B+tree.png" style="zoom:45%;" />

要注意的是，分裂是因为插入操作会导致其不符合B+树定义，才进行分裂；与B-树不同，它的分裂不是主动的。



- 删除操作
  1. 删除最简单的情况：只要在删除后结点内还有数据，就直接删除。
  2. B+树的合并也是被动的，只有当删除后结点内关键字不符合$\lceil m/2 \rceil≤n≤m$时，才进行合并操作，以索引取大及下图为例，删除97

 

- 在MySQL中常用的索引引擎是MyISAM（非聚集索引）、INNODB（聚集性索引）

<img src="Picture/5-13-4 B+tree.png" style="zoom:45%;" />

上述图是ISAM，索引文件与数据文件分开，而INNODB中的数据文件本身就是索引文件。如果以ASCII码，或者名字、一些英文字母，聚集索引非常高效。



#### 3.1.6 红黑树

其本质是一棵自平衡的二叉排序树，有以下红黑性质：
...
红黑树从以下角度进行了优化：
1、平衡操作：AVL树中进行插入和删除操作时，会频繁的进行旋转，而红黑树用“变色”操作来降低旋转的频度。但平衡树比红黑树更加的平衡。

那么如何让红黑树保持平衡呢？

- 黑高（黑色结点的高度）：从某个结点出发，一直到叶子结点，这条路径包含的黑色结点的数量。

##### 插入

- 插入操作主要是通过插入结点本身与其叔叔结点来进行变换，且是为了解决两个红色结点相连的问题

假设x是新插入的一个结点
1、先按照排序树的标准进行插入，设置为红色；
2、如果x是根结点，变色为黑色；
3、如果x的父结点不是黑色且不是根结点：
(1)如果叔叔结点是红色，就考虑着色：
...
③此时以爷爷结点为根基，重复①②，直到根节点为黑色即可；
(2)如果叔叔结点是黑色，就考虑旋转。

以下列插入图为示例演示：

 <img src="Picture/5-14-1 Red-Black-tree.png" style="zoom:45%;" />

插入结点6，并设置为红色，此时符合红黑树染色定义

<img src="Picture/5-14-2 Red-Black-tree.png" style="zoom:45%;" />

插入结点9，将其设置为红色

<img src="Picture/5-14-3x Red-Black-tree.png" style="zoom:45%;" />

其叔叔结点是黑色结点（NULL），此时情形为RR，进行左旋

 <img src="Picture/5-14-3 Red-Black-tree.png" style="zoom:45%;" />

旋转后，2与6的颜色互换，此时符合红黑树染色定义

<img src="Picture/5-14-4 Red-Black-tree.png" style="zoom:45%;" />

插入结点12

<img src="Picture/5-14-5 Red-Black-tree.png" style="zoom:45%;" />

其叔叔结点是红色，将父结点和叔叔结点都设置为黑色

<img src="Picture/5-14-6 Red-Black-tree.png" style="zoom:45%;" />

插入结点15

<img src="Picture/5-14-7 Red-Black-tree.png" style="zoom:45%;" />

叔叔结点是黑色（NULL结点不显示），属于RR，进行左旋

<img src="Picture/5-14-8 Red-Black-tree.png" style="zoom:45%;" />

旋转后，将结点12与结点9的染色情况互换

<img src="Picture/5-14-9 Red-Black-tree.png" style="zoom:45%;" />

插入结点20

<img src="Picture/5-14-10 Red-Black-tree.png" style="zoom:45%;" />

其叔叔结点是红色，将父结点和叔叔结点都设置为黑色，将父结点的父结点设置为红色，其父结点为根结点，不再向上重复这一过程。

<img src="Picture/5-14-11 Red-Black-tree.png" style="zoom:45%;" />

插入结点25

<img src="Picture/5-14-12 Red-Black-tree.png" style="zoom:45%;" />

其叔叔结点是黑色（NULL结点不显示），属于RR，进行左旋

<img src="Picture/5-14-13 Red-Black-tree.png" style="zoom:45%;" />

旋转后，将结点20与结点15的染色情况互换

<img src="Picture/5-14-14 Red-Black-tree.png" style="zoom:45%;" />

插入结点23

<img src="Picture/5-14-15 Red-Black-tree.png" style="zoom:45%;" />

此时叔叔结点为红色，则将父结点和叔叔结点都置为黑色，父结点的父结点置为红色

<img src="Picture/5-14-16 Red-Black-tree.png" style="zoom:45%;" />

此时看结点20，其父结点为红色，叔叔结点为黑色，属于RR，进行左旋

<img src="Picture/5-14-17 Red-Black-tree.png" style="zoom:45%;" />

将结点12与结点6的染色情况互换，得到下图

<img src="Picture/5-14-18 Red-Black-tree.png" style="zoom:45%;" />



##### 删除：

通过删除结点的兄弟结点来决定操作，删除操作主要是为了解决黑高的问题

（设删除的结点为v，替代结点是u，用左子树中的最大结点）

1、先执行基本的BST删除
2、如果v或者u是红色的，直接替换后删除

如要删除10，其替代结点是9

<img src="Picture/5-14-19 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-20 Red-Black-tree.png" style="zoom:45%;" />

3、如果v和u都是黑色的——（双黑结点：当被删除结点v是黑色的，被其他的黑色结点替换了，此时这个子结点就叫做双黑结点）
(1)如果替换结点u是黑色结点且不是根结点，此时看兄弟结点的颜色(如下，删除结点25)

<img src="Picture/5-14-21 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-22 Red-Black-tree.png" style="zoom:45%;" />

①替代结点是一个双黑结点，替代结点的兄弟是黑色，且兄弟的孩子至少有一个是红色结点（将上图的10和18分情况看，不在一张图中）
I. 如果兄弟结点的左孩子为红色

<img src="Picture/5-14-22 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-23 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-24 Red-Black-tree.png" style="zoom:45%;" />

II.如果兄弟结点的右孩子为红色，则先左旋再右旋。

<img src="Picture/5-14-25 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-26 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-27 Red-Black-tree.png" style="zoom:45%;" />

<img src="Picture/5-14-28 Red-Black-tree.png" style="zoom:45%;" />

②兄弟结点是黑色，其两个孩子结点也都是黑色（删除15）

<img src="Picture/5-14-28 Red-Black-tree.png" style="zoom:45%;" />

对删除结点的兄弟结点直接变色

<img src="Picture/5-14-29 Red-Black-tree.png" style="zoom:45%;" />

然后以18作为替代结点继续看18的兄弟结点，重复此过程向上看，直到父结点是一个红色结点，结束该过程。

③兄弟结点是红色（删除41）

<img src="Picture/5-14-30 Red-Black-tree.png" style="zoom:45%;" />

此时，仅通过变色不能解决黑高的问题，就要考虑旋转。

<img src="Picture/5-14-31 Red-Black-tree.png" style="zoom:45%;" />

旋转后对原先父结点与现在父结点对换颜色即可

<img src="Picture/5-14-32 Red-Black-tree.png" style="zoom:45%;" />
...
1. 就地排序，空间复杂度为$O(1)$
2. 内部排序和外部排序：区别在于待排序的关键字是否能一次性载入内存中
3. 稳定（数据和数据之间的相对位置）排序：

目前新语言的底层都不再使用快排，而是使用TimSort，是一种归并排序的变体，也是很常见的底层排序代码。
快排是一种不稳定的排序，其不稳定性也是逐渐不再底层中使用的原因之一，但是所有的不稳定排序都可以转化为稳定排序，只是在一般情况中不做处理，其原因是转化总是要牺牲空间，代价过大。

#### 3.2.1 插入排序

##### 3.2.1.1 直接插入排序

如下图所示，将一个数组分为有序和无序两个部分。（绿色代表有序，白色代表无序）


<img src="Picture/5-4-1 Insert Sort.png" style="zoom:67%;" />

将20与9进行对比，20＞9，直接将20纳入到有序序列中。

<img src="Picture/5-4-2 Insert Sort.png" style="zoom:67%;" />

将13与20对比，13＜20，再将其与9对比，13＞9，将20向后移动，13向前移动

<img src="Picture/5-4-3 Insert Sort.png" style="zoom:67%;" />

同上述步骤，最后得到：

<img src="Picture/5-4-4 Insert Sort.png" style="zoom:67%;" />

当待排序的数量＜8时，通常使用

##### 3.2.1.2 折半插入排序

- 其核心思想为直接插入+折半查找，相对于直接插入排序而言，它减少了一些比较的次数

##### 3.2.1.3 二路插入排序

- 减少移动的次数，环形数组；另外设置一个同存储记录大小的相同数组，将其作为排好序的数组，把无序数组（原数组）的第一个数字放到新数组中作为基准，较大的数字放在基准的右边，较小的数字放在基准的左边，将新数组视为环状数组，如下图所示

<img src="Picture/5-4-5 Insert Sort.png" style="zoom:67%;" />

将基准放在a[0]，10比9大，将10放在基准右边也就是a[1]的位置，8比9小，将8放在基准左边的位置，这里基准在a[0]位置，左移则用取余计算$\%maxSize$，计算得a[4]。

<img src="Picture/5-4-6 Insert Sort.png" style="zoom:67%;" />

##### 3.2.1.4 希尔排序（缩小增量排序）

使用到了跳跃式分组

<img src="Picture/5-5-1 Shell Sort.png" style="zoom:50%;" />

在学习分组中，我们一般使用$length/2$来进行分组，但是在实际工程中，希尔增量是一个变值，而不是定值；我们设增量$gap=length/2=5$组，缩小增量为$gap=gap/2$。

下图为分组示意

<img src="Picture/5-5-2 Shell Sort.png" style="zoom:50%;" />

每组之中进行插入排序

<img src="Picture/5-5-3 Shell Sort.png" style="zoom:50%;" />

缩小分量继续分组

<img src="Picture/5-5-4 Shell Sort.png" style="zoom:50%;" />

<img src="Picture/5-5-5 Shell Sort.png" style="zoom:50%;" />

最后将全部元素视为一组进行排序

<img src="Picture/5-5-6 Shell Sort.png" style="zoom:50%;" />

#### 3.2.2 交换排序

##### 3.2.2.1 冒泡排序

每一轮都将最大的元素沉底即可，其示意图如下（红色意味着已排好序的序列）：

<img src="Picture/5-6-1 bubble Sort.png" style="zoom:50%;" />

<img src="Picture/5-6-2 bubble Sort.png" style="zoom:50%;" />

<img src="Picture/5-6-3 bubble Sort.png" style="zoom:50%;" />

<img src="Picture/5-6-4 bubble Sort.png" style="zoom:50%;" />

<img src="Picture/5-6-5 bubble Sort.png" style="zoom:50%;" />

<img src="Picture/5-6-6 bubble Sort.png" style="zoom:50%;" />

- 对冒泡排序的第一次优化：当前面元素有序时就不再重复进行比较。解决方法为加一个标记变量，在比较过程中，没有发生交换过位置的元素，标记为1，当遇到标记为1时，brake。
- 对冒泡排序的第二次优化：当后面元素有序时就不再重复进行比较，用一个界分有序和无序即可，问题是如何记录界定的位置。

<img src="Picture/5-6-7 bubble Sort.png" style="zoom:50%;" />

- 用一个变量记录上述交换的位置（以记录前一个元素的位置为例）：如上图所示，4与2比较后需交换位置，该变量为0；4与1比较后需交换位置，变量更新为1，其余后面的比较均不需要交换位置，则在第一趟冒泡中变量更新为1，如下图所示。

<img src="Picture/5-6-8 bubble Sort.png" style="zoom:50%;" />

- 记录变量以后位置的元素均是有序的，无需再次比较。



##### 3.2.2.2 快速排序

(注：在考研和严蔚敏书中采用的是挖坑法，不是下述步骤，详见书中内容)

先选取一个基准（要把它放到排序后数组中正确的位置），根据基准的不同，快排的代码会不一样，这里以最后一个位置为基准来描述该排序过程。
思想：每一次进行的划分都可以看做是把基准的值放到数组中正确的位置上。
设置一个指针 $i$ 表示数组最后正确的位置，初始设置为 $i=-1$；设置另一个指针 $j$ 用来遍历数组，初始设置为 $j=0$，如下所示：

<img src="Picture/5-7-1 quick Sort.png" style="zoom:50%;" />

将1和基准7进行比较，1＜7，此时将 $i$ 向后移动一位，且数组无变化，遍历指针 $j$ 向后移动一位

<img src="Picture/5-7-2 quick Sort.png" style="zoom:50%;" />

将8和7进行比较，8＞7，此时 $i$ 不动，数组不发生变化，$j$ 向后移动一位

<img src="Picture/5-7-3 quick Sort.png" style="zoom:50%;" />

将3和7进行比较，3＜7，此时将 $i$ 向后移动一位，此时将 $i$ 和 $j$ 的值交换，$j$ 向后移动一位。

<img src="Picture/5-7-4 quick Sort.png" style="zoom:50%;" />

将9和7进行比较，9＞7，此时 $i$ 不动，数组不发生变化，$j$ 向后移动一位

<img src="Picture/5-7-5 quick Sort.png" style="zoom:50%;" />

将4和7进行比较，4＜7，此时将 $i$ 向后移动一位，此时将 $i$ 和 $j$ 的值交换，$j$ 向后移动一位。

<img src="Picture/5-7-6 quick Sort.png" style="zoom:50%;" />

将5和7进行比较，5＜7，此时将 $i$ 向后移动一位，此时将 $i$ 和 $j$ 的值交换，$j$ 向后移动一位。

<img src="Picture/5-7-7 quick Sort.png" style="zoom:50%;" />

将4和7进行比较，4＜7，此时将 $i$ 向后移动一位，此时将 $i$ 和 $j$ 的值交换，$j$ 向后移动一位。

<img src="Picture/5-7-8 quick Sort.png" style="zoom:50%;" />

此时遍历结束，$i$ 向后移动一位，将 $i$ 和 $j$ 的值交换，这时我们就找到了基准7在数组中的正确位置

<img src="Picture/5-7-9 quick Sort.png" style="zoom:50%;" />

7前面的值都比7小，7后面的值都比7小，此时分为两块，单独对两块都进行快排（分治）

<img src="Picture/5-7-10 quick Sort.png" style="zoom:50%;" />

剩下的部分同上述步骤，直到所有元素排序完即可。最坏情况下，对于有序数组来说，其递归过程退化成了一个单链，没有分治的部分；最好情况下，每次都能选到正中间，就会变成平衡二叉树。



#### 3.2.3 选择排序

选择排序跟直接插入排序几乎一模一样，以下图为例，先在所有的数里找到最小的一位，然后让其跟第一位置上的元素交换。

<img src="Picture/5-8-1 select Sort.png" style="zoom:50%;" />

交换完后，左边则有序，右边重复进行上述操作即可。

<img src="Picture/5-8-2 select Sort.png" style="zoom:50%;" />



#### 3.2.4 归并排序（分治）

<img src="Picture/5-9-1 Merge Sort.png" style="zoom:50%;" />

首先对上述进行划分，$(头指针+尾指针)/2$即可，如下划为0-2，3-5两个区间

<img src="Picture/5-9-2 Merge Sort.png" style="zoom:50%;" />



继续对这两块继续划分，划分后的结果如下所示

<img src="Picture/5-9-3 Merge Sort.png" style="zoom:50%;" />

此时，我们对划分好的结果进行排序

<img src="Picture/5-9-4 Merge Sort.png" style="zoom:50%;" />

归并排序是在用空间换时间，其核心思想就是在分的过程，至于治的过程，用之前的排序即可。上述所演示的归并排序叫做二路归并排序。



#### 3.2.5 计数排序

计数排序的思想非常简单，顾名思义就是数数，以下图为例

<img src="Picture/5-10-1 Counting Sort.png" style="zoom:50%;" />

找到其中最大的数字，并按照该数字+1开辟一个新的数组用以计数，这里开辟新数组的大小是9。

<img src="Picture/5-10-2 Counting Sort.png" style="zoom:50%;" />

按照输出个数输出即可，通过此可以发现，计数排序是一个**不稳定**的排序，一般来说被描述成一个**稳定**的排序。

如果要使得其是一个稳定的排序，则严格按照下述步骤操作：

1. 统计每个元素出现的次数。
2. 计算累积计数（每个元素在输出数组中的结束位置）。
3. **从后向前**遍历原始数组，将每个元素放到输出数组的对应位置，并相应减少累积计数。

在前文中提到过，任何一个不稳定的排序都可以变稳定，只是一般变稳定是代价很大的，而计数排序变稳定是相对简单的，先看根据Counting所得输出的数组。

<img src="Picture/5-10-3 Counting Sort.png" style="zoom:50%;" />

输出的数组不能体现数据的先后，使用一个公式$count[i]=count[i]+count[i-1]$，Counting数组则变化为

<img src="Picture/5-10-4 Counting Sort.png" style="zoom:50%;" />

$i$ 代表数组下标，$i=n-1=9$，$arr[9]=4$，在排序后数组中的正确位置是 $count[arr[i]]-1=7$，原本在arr[9]的数组就应该在arr[7]，这样就能保证数据的先后性使其达到稳定，这里开辟一个新的数组来记录稳定后的位置。

<img src="Picture/5-10-5 Counting Sort.png" style="zoom:50%;" />



#### 3.2.6 桶排序

桶排序（Bucket Sort）是一种基于**分布**的排序算法，其核心思想是将待排序的数据**分散到有限数量的桶中**，每个桶再分别进行排序（通常借助其他排序算法或递归方式），最后按顺序合并各个桶中的元素，从而得到全局有序序列。

桶排序的基本思想（空间换时间）：

1. **划分桶**：根据数据的取值范围，设置若干个桶（通常用数组或链表表示），每个桶对应一个区间。
2. **分配元素**：遍历待排序序列，根据某种映射函数将每个元素放入对应的桶中。映射函数需要保证元素均匀分布到各个桶。
3. **桶内排序**：对每个非空桶内的元素进行排序（可使用插入排序、快速排序等，或递归使用桶排序）。
4. **合并桶**：按桶的顺序依次输出各桶内的元素，即得到有序序列。



#### 3.2.7 基数排序

通过逐位分配收集实现排序，时间复杂度O(d*(n+k))。其核心优势在于避免直接比较，特别适用于多关键字排序（如日期数据）或固定位数的整数排序，以及对稳定性有要求的分层排序场景。当数据范围可控且位数较少时，可达到近似线性的效率。

如：170、45、75、90、800、24、2、66进行基数排序，按照个、十、百的顺序依次来进行排序。

<img src="Picture/5-11-1 Radix Sort.png" style="zoom:36%;" />

个位排序后：170、90、800、2、24、45、75、66

<img src="Picture/5-11-2 Radix Sort.png" style="zoom:36%;" />

十位排序后：800、2、24、45、66、170、75、90

<img src="Picture/5-11-3 Radix Sort.png" style="zoom:36%;" />

百位排序后：2、24、45、66、75、90、170、800
...
对于根节点比其孩子结点大的，叫做大顶堆；反之为小顶堆。对堆而言，只比较其上下，不管左右，下图示例是一个正确的堆排序。

<img src="Picture/5-15-1 heap sort.png" style="zoom:36%;" />

现在用数组来表示二叉堆，另起左孩子在[2x]上，右孩子在[2x+1]上。

<u>堆化操作</u>：对堆进行调整，调整至想要的结构（大顶堆或小顶堆），每3个结点作对比，这个操作不一定是从上到下的，比如说新插入结点会从下到上去堆化。

- 插入操作的堆化操作上从下到上的

- 删除操作：先把待删除结点与根节点交换，且将其置为无穷小，然后将最后一个结点替换到根节点中，从上到下进行堆化操作。

二项式堆、斐波那契堆都是二叉堆的变体；堆排序在工程中的较为常用的应用之一就是**优先级**队列。
...
next数组的作用就是找到需要回溯的位置。`next[j] = j`，next[j]就是表示对于子串而言，前$j-1$个字符的前后缀相同的最大长度+1，规定next数组第一个位置为0。
...
空间复杂度，开辟出来存放具体数据的数组或空间不将其纳入空间复杂度中；当处理问题时，需要开辟其他空间来做辅助的那些空间，将其纳入为空间复杂度中，且空间复杂度为$O(1)$表示其所需辅助空间大小与问题规模$n$无关，而不代表不需要任何辅助空间。
