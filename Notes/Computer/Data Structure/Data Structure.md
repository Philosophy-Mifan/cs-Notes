...
- 直接插入排序：如下图所示，将一个数组分为有序和无序两个部分。（绿色代表有序，白色代表无序）
...
- 折半插入排序：其核心思想为直接插入+折半查找，相对于直接插入排序而言，它减少了一些比较的次数

- 二路插入排序：减少移动的次数，环形数组；另外设置一个同存储记录大小的相同数组，将其作为排好序的数组，把无序数组（原数组）的第一个数字放到新数组中作为基准，较大的数字放在基准的右边，较小的数字放在基准的左边，将新数组视为环状数组，如下图所示
...
- 希尔排序（缩小增量排序）：使用到了跳跃式分组

<img src="Picture/5-5-1 Shell Sort.png" style="zoom:50%;" />

在学习分组中，我们一般使用$length/2$来进行分组，但是在实际工程中，希尔增量是一个变值，而不是定值
